...
예를 들면 {% if current_user.is_authenticated %} <- 이게 맞는 문장인데 {% if current_user.is_authenticated() %} 이런식으로 혼란을 준다거나...
정작 git checkout 버전넘버(8c)를 입력해서 해당 부분을 검토하면 괄호가 사라져있고... 내용을 잘 진행하다가 사소한 이러한 문제 때문에 힘들다. 
